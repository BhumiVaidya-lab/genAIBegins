...
/**
 * Service class for processing text into embeddings and interacting with Qdrant for vector storage
 * and retrieval.
 *
 * <p>This service converts text into embeddings using Azure OpenAI and saves these vectors in a
 * Qdrant collection. It also provides functionality to searchEmbeddings for similar vectors based on input
 * text.
 */
@Slf4j
@Service
...
public class SimpleVectorActions {
    ...
    /**
     * Processes the input text into embeddings, transforms them into vector points, and saves them in
     ...
     *
     * <p>The input text is converted to embeddings, and a searchEmbeddings is performed based on the vector
     * similarity.
     *
     * @param text the text to searchEmbeddings for similar vectors
     * @return a list of scored points representing similar vectors
     * @throws ExecutionException if the searchEmbeddings operation fails
     * @throws InterruptedException if the thread is interrupted during execution
     */
    public List<ScoredPoint> search(String text) throws ExecutionException, InterruptedException {
        ...
        Objects.requireNonNull(embeddings.block())
                .getData()
                .forEach(embeddingItem -> qe.addAll(embeddingItem.getEmbedding()));
        return qdrantClient
                .searchAsync(
     ...
     */
    private void saveVector(ArrayList<PointStruct> pointStructs)
